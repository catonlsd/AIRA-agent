...
Entries record intentionally deferred design-system, accessibility, and frontend
architecture work. Counts are discovery-time snapshots unless an entry explicitly
states that it was refreshed during release review.
...
**Target phase:** Phase 6B — States & Accessibility
...
**Target phase:** Phase 6A — Visual Consistency
...
Deferred to: Phase 6A — Visual Consistency
...
**Category:** Deprecated surface and radius aliases remain in active use
**Instances:** 288 callsites at the Phase 2 release review (284 legacy surface
alias consumers and 4 legacy radius alias consumers)
**Discovered:** Release review — Phase 2
**Target phase:** Phase 6A — Visual Consistency
**Description:** The Phase 1 contract deprecated `--surface`, `--surface-soft`,
`--surface-muted`, `--surface-strong`, `--radius`, and `--radius-pill`, but the
later frozen phases did not complete their consumer migration. Removing the aliases
now would break existing components. Phase 6A must migrate and visually verify all
consumers across the supported themes before deleting the compatibility aliases.
